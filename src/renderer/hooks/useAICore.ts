...
function normalizeStats(backendStats: any): AIStatusType['stats'] {
  return {
...
function normalizeStatus(backendStatus: any): AIStatusType | null {
...
export function useAICore() {
  ...
  const fetchStatus = useCallback(async () => {
    if (!window.flowrider?.leoai) {
      console.warn('[useAICore] AI System service not available');
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      console.error('[useAICore] Failed to fetch status:', err);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const fetchPatterns = useCallback(async (minConfidence = 0.5) => {
    ...
    try {
      ...
      console.error('[useAICore] Failed to fetch patterns:', err);
    }
  }, []);
  ...
  const fetchInsights = useCallback(async (limit = 20) => {
    ...
    try {
      ...
      console.error('[useAICore] Failed to fetch insights:', err);
    }
  }, []);
  ...
  const fetchLearningEvents = useCallback(async () => {
    ...
    try {
      ...
      console.error('[useAICore] Failed to fetch learning events:', err);
    }
  }, [lastEventCheck]);
  ...
  const getContext = useCallback(async (options: {
    ...
    try {
      ...
      console.error('[useAICore] Failed to get context:', err);
    }
    return null;
  }, []);
  ...
  const getQuickContext = useCallback(async (projectId?: string, language?: string) => {
    ...
    try {
      ...
      console.error('[useAICore] Failed to get quick context:', err);
    }
    return null;
  }, []);
  ...
  const triggerAnalysis = useCallback(async () => {
    ...
    try {
      ...
      console.error('[useAICore] Failed to trigger analysis:', err);
    }
    return null;
  ...
  const recordFeedback = useCallback(async (
    ...
    try {
      ...
      console.error('[useAICore] Failed to record feedback:', err);
      return false;
    }
  }, []);
  ...
  const enable = useCallback(async () => {
    ...
    try {
      ...
      console.error('[useAICore] Failed to enable:', err);
      return false;
    }
  }, [fetchStatus]);
  ...
  const disable = useCallback(async () => {
    ...
    try {
      ...
      console.error('[useAICore] Failed to disable:', err);
      return false;
    }
  }, [fetchStatus]);
