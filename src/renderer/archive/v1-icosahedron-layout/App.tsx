...
const App: React.FC = () => {
  ...
  const { sessions, selectedFace, selectFace, setAttachedSession, updateSession, costMetrics, appMode, setAppMode, resetDemoData, syncWithTmux } = store;

  console.log('[App] Store loaded, sessions:', sessions?.length);

  // Mark store as ready after first render and auto-select first face
  useEffect(() => {
    console.log('[App] Store mounted, marking ready');
    setStoreReady(true);

    // Auto-select face 0 if nothing is selected so user sees session controls immediately
  ...
  const handleFaceClick = (faceIndex: number) => {
    ...
    // Auto-attach when selecting a face with an active tmux session
